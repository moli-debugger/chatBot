...
export class MessageBoxComponent implements OnInit {
  public Main: any = document.getElementsByTagName("main");
  public question = "";
  public text = "";
  public Answer: any;
  public User: any;
  ...
  public addQuestion(main: any, text: any) {
    console.log("main", main, "msg:", text)
    main[0].innerHTML += `
        <div class="row">
  ...
  send(message: string) {
    ...
    this.addQuestion(this.Main, message);
    this.question = "";
    this.Answer(this.Main, this.userInfo.userName, this.text);
  }
  ...
  flow() {
    ...
    const ListThings = (ThingsICanDo: any[]) => "You can try asking the following: <br><br><br>" + ThingsICanDo.join("<br/><br/>");

    const Random = (list: string | any[]) => list[Math.floor(Math.random() * list.length)];

    const WhatCanIDo = (ThingsICanDo: string[]) => Random(ThingsICanDo);
    ...
    //const getLink = (siteName: string) => `Click here to open <a href="${Links[siteName]}" target="_blank">${siteName}</a>`;
    ...
    const TellAJoke = (Jokes: string[]) => Random(Jokes);
    const Facts = [
    ...
    const TellAFacts = (Facts: string[]) => Random(Facts);
    ...
    const TellAStory = (Story: string[]) => Random(Story);
    ...
    const isLegal = (string: string | string[]) => {
    ...
    const HtmlSpecialChars = (string: any) => {
    ...
    const element = (selector: string) => D.querySelector(selector);

    this.Answer = (Main: any, User: string | null, Text: string) => {
      let Flag = false;
    ...
    const addAnswer = (Main: { innerHTML: string; }, text: string) => {
      Main.innerHTML += `
      <div class="row">
    ...
    D.addEventListener("DOMContentLoaded", () => {
      ...
      // const Question: any = document.getElementsByTagName("input");

      const Lastdiv = document.querySelector("#last");

      this.userInfo.userName = prompt("Enter your name :");

      while (this.userInfo.userName === null || this.userInfo.userName === '') {

        this.userInfo.userName = prompt("Your name is required for the assistant to work properly : ");
      ...
      const Ask = () => {
        ...
        if (Text.length) {
          ...
          ///this.addQuestion(Main, Text);

          //Question.value = "";

          //this.Answer(this.Main, User, this.text);
      ...
      // Question.addEventListener("keyup", (event: { keyCode: number; }) => {
      //   if (event.keyCode === 13) Ask();
      // });

      addAnswer(this.Main, `Hello ${this.userInfo.userName}, I am Trek. How can I help`);
